...
object A extends App {
  ...
  @annotation.tailrec
  def go(maximum: Int, count: Int = 0): Int =
    nextInt() match {
      case 0                                        => count
      case number if count == 0 || number > maximum => go(number, 1)
      case number if number == maximum              => go(maximum, count + 1)
      case _                                        => go(maximum, count)
    }
